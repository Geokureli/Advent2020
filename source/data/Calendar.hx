...
class Calendar
{
    ...
    inline static function setDebugDay(callback:()->Void)
    {
        ...
        {
            case DAY(32, _):
                onDateReceived(Date.fromString("2021-01-01"), callback);
            case DAY(n, _) if (n < 10):
                onDateReceived(Date.fromString('2020-12-0$n'), callback);
            case DAY(n, _):
                onDateReceived(Date.fromString('2020-12-$n'), callback);
            case STRING(date, _):
